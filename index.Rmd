...
Total
================================
### <font size="5"> **Total do Nº de concluintes em ensino superior - Sexo Feminino** </font>


```{r,eval=TRUE}

CONCLUINTES_SSA %>% filter(TP_SEXO %in% c("F")) %>% group_by(ANO_CENSO_SUP) %>% summarise(value =sum(value/1000)) %>% mutate(ANO_CENSO_SUP=as.character(ANO_CENSO_SUP)) %>%
