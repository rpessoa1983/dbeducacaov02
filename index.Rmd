...
objeto <- CONCLUINTES_SSA %>% filter(TP_COR_RACA %in% c("Preta","Parda")) %>% filter(TP_SEXO %in% c("F")) %>% group_by(ANO_CENSO_SUP) %>% summarise(value =sum(value)) %>%
ggplot(aes(x=ANO_CENSO_SUP,y=value)) +
  geom_bar(stat = "identity",fill="#00c4cc",color="#fac113",width = 0.5) + 
  labs(
    title = "",
...
### <font size="5"> **Nº de concluintes em ensino superior - Sexo Feminino** </font>

```{r, eval=TRUE, fig.height=4, fig.width=3}

objeto <- CONCLUINTES_SSA %>% filter(TP_SEXO %in% c("F")) %>% group_by(ANO_CENSO_SUP) %>% summarise(value =sum(value)) %>%
ggplot(aes(x=ANO_CENSO_SUP,y=value)) +
  geom_bar(stat = "identity",fill="#00c4cc",color="#fac113",width = 0.5) + 
  labs(
    title = "",
